...
describe('Build System', () => {
    ...
    describe('Build command execution', () => {
        it('should execute pnpm build commands for each package', () => {
            ...
            packages.forEach(pkg => {
                expect(mockExecSync).toHaveBeenCalledWith(`pnpm --filter=@freedom/${pkg} run build`, { stdio: 'inherit', cwd: process.cwd() });
            });
        });
    ...
    describe('Build script output', () => {
        ...
        it('should provide error output on failure', () => {
            ...
            expect(mockConsoleError).toHaveBeenCalledWith('❌ Failed to build package cli:', 'Build error message');
        });
    });
